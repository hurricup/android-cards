...
- **Scheduling (Leitner):** `model/TierScheduler.kt` + `TierStore.kt` + `Tiers.kt`. Each question has a tier + last-answered time stored in `tiers/<id>.json` (per questionary id). Correct → next tier (a correct first answer on a new card jumps to tier 2); wrong → reset to tier 1; tier 0 = unknown/new. Tier 1 is always due (recurs until it graduates); higher tiers due after their interval. Session = due cards (highest tier first, max tier capped) then new to fill. Interval per tier = `multiplier^(tier-1)` days, multiplier configurable in the settings gear. Pie chart shows per-tier distribution.
